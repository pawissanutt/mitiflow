...
async fn e2e_3_nodes_full_coverage() {
    let mut cluster = TestCluster::new("e2e_3n_full", 3, 12, 1).await;
    for i in 0..3 {
...
async fn e2e_3_nodes_rf2_coverage() {
    let mut cluster = TestCluster::new("e2e_3n_rf2", 3, 6, 2).await;
    for i in 0..3 {
        cluster.start_agent(i).await;
    }
    cluster.wait_for_stable(Duration::from_millis(1500)).await;
...
async fn e2e_node_join_rebalances() {
    let mut cluster = TestCluster::new("e2e_join", 3, 8, 1).await;
...
async fn e2e_node_join_minimal_disruption() {
    let mut cluster = TestCluster::new("e2e_join_min", 3, 12, 1).await;
...
async fn e2e_graceful_leave_rebalances() {
    let mut cluster = TestCluster::new("e2e_leave", 3, 6, 1).await;
    for i in 0..3 {
        cluster.start_agent(i).await;
    }
    cluster.wait_for_stable(Duration::from_millis(1500)).await;
...
async fn e2e_crash_and_rejoin() {
    let mut cluster = TestCluster::new("e2e_crash_rejoin", 3, 6, 1).await;
    for i in 0..3 {
        cluster.start_agent(i).await;
    }
    cluster.wait_for_stable(Duration::from_millis(1500)).await;
...
async fn e2e_single_node_cluster() {
    let mut cluster = TestCluster::new("e2e_single", 1, 4, 1).await;
    cluster.start_agent(0).await;
...
async fn e2e_all_nodes_restart() {
    let mut cluster = TestCluster::new("e2e_restart_all", 3, 6, 1).await;
    for i in 0..3 {
        cluster.start_agent(i).await;
    }
    cluster.wait_for_stable(Duration::from_millis(1500)).await;
...
async fn e2e_rapid_join_leave() {
    let mut cluster = TestCluster::new("e2e_rapid", 5, 8, 1).await;
...
async fn e2e_data_survives_graceful_leave() {
    ...
    let mut cluster = TestCluster::new("e2e_data_leave", 2, 4, 1).await;
    cluster.start_agent(0).await;
    cluster.start_agent(1).await;
    cluster.wait_for_stable(Duration::from_millis(1500)).await;

    // Publish 5 events to every partition using a session joined to the
    // cluster's isolated network.
    let pub_domain = cluster.join_session().await;
    let publisher = cluster.create_publisher(pub_domain.session()).await;
    for p in 0..4u32 {
    ...
    for p in 0..4u32 {
        let count = cluster.query_store_count(pub_domain.session(), p).await;
        found_total += count;
    }
    ...
    drop(publisher);
    pub_domain.shutdown().await.unwrap();
    cluster.shutdown_all().await;
}
...
async fn e2e_data_live_ingestion() {
    ...
    let mut cluster = TestCluster::new("e2e_data_live", 2, 4, 1).await;
    cluster.start_agent(0).await;
    cluster.start_agent(1).await;
    cluster.wait_for_stable(Duration::from_millis(1500)).await;
    ...
    for p in 0..4u32 {
        let count = cluster
            .wait_for_events(pub_domain.session(), p, 1, Duration::from_secs(10))
            .await;
        found_total += count;
    }
...
async fn e2e_publish_during_rebalance() {
    let mut cluster = TestCluster::new("e2e_data_rebalance", 3, 4, 1).await;
    cluster.start_agent(0).await;
    cluster.start_agent(1).await;
    cluster.wait_for_stable(Duration::from_millis(1500)).await;
...
async fn e2e_data_survives_crash_and_recovery() {
    let mut cluster = TestCluster::new("e2e_data_crash", 2, 4, 1).await;
    cluster.start_agent(0).await;
    cluster.start_agent(1).await;
    cluster.wait_for_stable(Duration::from_millis(1500)).await;
